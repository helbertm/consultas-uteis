...
DECLARE @DatabaseName NVARCHAR(128), @Query NVARCHAR(MAX), @Version NVARCHAR(128);
...
BEGIN 
	...
	--Owner: engeman
	SET @Query = 'INSERT INTO #TempVersion SELECT VERSAO FROM [' + @DatabaseName + '].[engeman].CFGGERAL';
	BEGIN TRY
			EXEC sp_executesql @Query;
			SELECT @Version = VERSAO FROM #TempVersion;
	END TRY
	BEGIN CATCH
	...
	--Owner: dbo
	SET @Query = 'INSERT INTO #TempVersion SELECT VERSAO FROM [' + @DatabaseName + '].[dbo].CFGGERAL';
	BEGIN TRY
			EXEC sp_executesql @Query;
			SELECT @Version = VERSAO FROM #TempVersion;
	END TRY
	...
	END CATCH
		 ...
		 --Owner: nome do banco
	SET @Query = 'INSERT INTO #TempVersion SELECT VERSAO FROM [' + @DatabaseName + '].[' + @DatabaseName + '].CFGGERAL';
	BEGIN TRY
			EXEC sp_executesql @Query;
			SELECT @Version = VERSAO FROM #TempVersion;
	END TRY
	...
	SET @Query = 'INSERT INTO #TempVersion SELECT VERSAO FROM [' + @DatabaseName + '].[' + @DatabaseName + '_hml].CFGGERAL';
	BEGIN TRY
			EXEC sp_executesql @Query;
			SELECT @Version = VERSAO FROM #TempVersion;
	END TRY
	...
	INSERT INTO #DatabaseVersions (DatabaseName, Version) VALUES (@DatabaseName, @Version);
...
SELECT
	...
	CAST(SUM(mf.size) * 8 / 1024.0 / 1024.0 AS DECIMAL(10, 2)) AS "Tamanho-GB",
	dv.Version "Versão"
FROM
	sys.databases db
INNER JOIN
...
LEFT JOIN
	#DatabaseVersions dv ON db.name = dv.DatabaseName
WHERE
